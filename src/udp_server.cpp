...
udp_server::udp_server(const std::string &addr, int port)
  ...
  std::cout<<"binding result " << errno << "socket value "<< f_socket << std::endl;

  u_int val = 1;
  if(setsockopt(f_socket, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val)) < 0) {
  ...
  std::cout<<"binding result " << errno << ". return value " << r << std::endl;
